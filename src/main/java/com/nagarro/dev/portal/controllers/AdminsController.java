...
public class AdminsController {
	...
	public List<Ticket> getTickets(@RequestBody LoginForm data) {	// Get all tickets (Works)
		if (adminService.isValid(data)) {
			...
			return ((List<Ticket>)ticketRepository.findAll()).stream().map(x->{
	...
	@PostMapping("/updateTicket/{status}/{id}")
	@ResponseStatus(HttpStatus.ACCEPTED)
	public ResponseEntity<?> updateTicket(@PathVariable("id") long id, @PathVariable("status") String status, @RequestBody LoginForm data) {	// Update a ticket (Works)
